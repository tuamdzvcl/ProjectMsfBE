...
namespace projectDemo.Service.TicketTypeService
{
    ...
    public class TypeTicketService :ITypeTicketService
    {
        ...
        public TypeTicketService(ITypeTicketRepositorys ticketRepositorys, IMapper mapper,IEventRepository eventRepository)
        {
        ...
        public async Task<ApiResponse<TypeTickResponse>> UpdateTypeTicket(int TypeTickectID, TypeTicketUpdate request)
        {
            ...
            if (typeticket == null)
            {
                return ApiResponse<TypeTickResponse>.FailResponse(Entity.Enum.EnumStatusCode.SERVER, "Không tìm thấy typeTicketID");
            }

            var entity = _mapper.Map(request, typeticket );

            var update  = _ticketRepositorys.UpdateTicket(entity);
            var response = _mapper.Map<TypeTickResponse>(update);
            return ApiResponse<TypeTickResponse>.SuccessResponse(Entity.Enum.EnumStatusCode.SERVER, response);
        }
    }
}
